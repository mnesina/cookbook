...
```bash
if ($args ~* '(select|union|update|insert|table|...ets...|ascii|hex|unhex|drop)' ) { return 403; }
```
...
foreach ($queryStringArr as $q) {
    $qArr = explode("=", $q);
    if(isset($qArr[1])) {
        if (preg_match('/select|update|delete|insert|table|...ets...|union|join|hex|unhex|drop/i',$qArr[1]))
